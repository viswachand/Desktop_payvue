...
import { setupAutoLogout, clearAutoLogout } from "@/utils/setupAutoLogout";
...
>("auth/fetchCurrentUser", async (_, { rejectWithValue }) => {
  try {
    const data = await safeApiCall<User>(() => API.get("/api/users/currentuser"));
...
>("auth/initializeAuth", async (_, { rejectWithValue }) => {
  try {
    ...
    return {
      token: stored.token,
      user: Array.isArray(verifiedUser) ? verifiedUser[0] : verifiedUser,
    };
  } catch (error) {
    clearAuth();
    return rejectWithValue(extractAxiosErrorMessage(error));
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(loginUser.fulfilled, (state, action) => {
        ...
        state.success = true;
        setupAutoLogout(action.payload.token); // ⏳ schedule auto logout
      })
      ...
      .addCase(initializeAuth.fulfilled, (state, action) => {
        ...
        state.currentUser = action.payload.user;
        state.isAuthenticated = true;
        setupAutoLogout(action.payload.token); // ⏳ restore auto logout timer
      })
...
export const selectIsAdmin = (state: RootState) => state.auth.currentUser?.isAdmin ?? false;
